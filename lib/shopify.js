...
export async function createCheckoutWithConfigurations(cartItems) {
  // First, map your cart items to the format expected by the Draft Orders API,
  // including calculating the adjusted price based on configurations.
  const lineItems = cartItems.map(item => {
    ...
    // Note: The Draft Orders API allows specifying the price directly for each line item.
    return {
      variantId: item.id,
      quantity: item.variantQuantity,
      appliedDiscount: {
        ...
        description: "Price adjusted based on custom configurations",
        value: totalPrice - parseFloat(item.variantPrice), // Discount amount based on the base price
