...
export async function getOrderDetails(customerAccessToken, orderId) {
    const query = `
        {
            order(id: "${orderId}") {
                lineItems(first: 250) {
                ...
                processedAt
                orderNumber
                // Assuming there's a way to get the confirmation URL directly
                // This might not be directly available via the Storefront API
                // confirmationUrl
            }
        }`;

    const response = await ShopifyCustomerData(query, { customerAccessToken, orderId });
